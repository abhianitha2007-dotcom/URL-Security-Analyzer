...
def run_all_checks(url):
    """
    Runs every URL security module.
    ...
    url_length, length_status, length_score = (
    ...
    entropy, entropy_status, entropy_score = (
        check_entropy(url)
    )

    redirect_count, final_url, redirect_status, redirect_score = (
        check_redirects(url)
    )

    port, port_status, port_score = (
        check_port(url)
    )

    query_count, suspicious_parameters, query_status, query_score = (
        check_query_parameters(url)
    )

    homograph_found, homograph_status, homograph_score = (
        check_homograph(url)
    )

    typosquatting_found, typosquatting_status, typosquatting_score = (
        check_typosquatting(url)
    )

    punycode_found, punycode_status, punycode_score = (
        check_punycode(url)
    )

    javascript_patterns, javascript_status, javascript_score = (
        check_javascript(url)
    )

    missing_headers, headers_status, headers_score = (
        check_security_headers(url)
    )

    form_issues, form_status, form_score = (
        check_forms(url)
    )

    content_patterns, content_status, content_score = (
        check_content(url)
    )

    favicon_url, favicon_status, favicon_score = (
        check_favicon(url)
    )

    file_extension, file_status, file_score = (
        check_file_extension(url)
    )

    detected_emails, email_status, email_score = (
        check_email_address(url)
    )

    similar_words, similarity_status, similarity_score = (
        check_domain_similarity(url)
    )

    domain_age_score = (
        20
        if domain_age.get("confirmed_new")
        else 0
    )
